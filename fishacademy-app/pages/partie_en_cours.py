...
if not st.session_state['setting_ok'] :
    ...
    if col1.button(":floppy_disk: Enregistrer le draft", type="primary") :
        res_ = save_draft_csv(df_session)
        st.toast(f":floppy_disk: {res_} ligne sauvegardées.")
    if col2.button(":no_entry: Supprimer le draft", type="secondary") :
        delete_draft_csv()
        st.toast(":no_entry: Draft supprimé !")
    if col3.button(":white_check_mark: Valider la session", type="primary") :
